...
public class Main {
    ...
    public static final String CLIENT_NAME = "DiversityRSPS.jar";
    public static final String CLIENT_URL = "http://107.178.102.30:8080/loader/"+CLIENT_NAME;
    public static String CLIENT_PATH = System.getProperty("user.home") + File.separator + ".Diversity" + File.separator;
    ...
    public Main() throws Exception {
        try {
            ...
            frame = new JFrame("Diversity RSPS Launcher");
            ...
            File file = update();
            if (file == null) {
                JOptionPane.showMessageDialog(frame, "An error has occured while downloading Diversity RSPS.", "Error", JOptionPane.ERROR_MESSAGE);
                System.exit(1);
                return;
            }
            launch(file);
    ...
    private File update() throws Exception {
        ...
        File file = new File(CLIENT_PATH + CLIENT_NAME);
        ...
        Callable<File> task = () -> {
            for (int i = 0; i < 10; i++) {
                try {
                    ...
                    File newFile = new File(CLIENT_PATH + CLIENT_NAME);
                    if (!newFile.exists())
